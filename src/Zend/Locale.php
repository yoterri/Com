...
class Locale
{
     ...
     * @throws Com\Zend\Locale\Exception When autodetection has been failed
     */
    ...
    public static function getDefault()
    {
        if ((self::$compatibilityMode === true) or (func_num_args() > 0)) {
            if (!self::$_breakChain) {
                ...
                trigger_error('You are running Com\Zend\Locale in compatibility mode... please migrate your scripts', E_USER_NOTICE);
     ...
     *
     * @param  string|Com\Zend\Locale $locale  Locale to set
     * @param  float              $quality The quality to set from 0 to 1
     * @throws Com\Zend\Locale\Exception When a autolocale was given
     * @throws Com\Zend\Locale\Exception When a unknown locale was given
     * @return void
     */
    public static function setDefault($locale, $quality = 1)
    {
        if (($locale === 'auto') or ($locale === 'root') or ($locale === 'default') or
            ...
            throw new Com\Zend\Locale\Exception('Only full qualified locales can be used as default!');
        }

        if (($quality < 0.1) or ($quality > 100)) {
            
            throw new Com\Zend\Locale\Exception("Quality must be between 0.1 and 100");
        }
        ...
        if (isset(self::$_localeData[(string) $locale]) === true) {
            ...
            if (isset(self::$_localeData[$elocale[0]]) === true) {
                ...
                throw new Com\Zend\Locale\Exception("Unknown locale '" . (string) $locale . "' can not be set as default!");
            }
        }
        ...
        foreach ($languages as $locale) {
            ...
            if ($language !== 'C') {
                ...
                $language = str_ireplace(
                    array_keys(Com\Zend\Locale\Data\Translation::$languageTranslation),   
                    array_values(Com\Zend\Locale\Data\Translation::$languageTranslation),
                    (string) $language
                );

                $language = str_ireplace(
                    array_keys(Com\Zend\Locale\Data\Translation::$regionTranslation),
                    array_values(Com\Zend\Locale\Data\Translation::$regionTranslation),
                    $language
                );
     ...
     * Sets a new locale
     *
     * @param  string|Com\Zend\Locale $locale (Optional) New locale to set
     * @return void
     */
     ...
     *
     * @param  Com\Zend\Locale $object Locale to check for equality
     * @return boolean
     */
    public function equals(Com\Zend\Locale $object)
    {
     ...
     * @param  string|Com\Zend\Locale $locale (Optional) Locale|Language for which this informations should be returned
    ...
    public static function getTranslationList($path = null, $locale = null, $value = null)
    {
        ...
        $result = Com\Zend\Locale\Data::getList($locale, $path, $value);
     ...
     *
     * @param  string|Com\Zend\Locale $locale (Optional) Locale for language translation
     * @return array
     * @deprecated
     */
     ...
     *
     * @param  string|Com\Zend\Locale $locale (Optional) Locale for script translation
     * @return array
     * @deprecated
     */
     ...
     *
     * @param  string|Com\Zend\Locale $locale (Optional) Locale for country translation
     * @return array
     * @deprecated
     */
     ...
     *
     * @param  string|Com\Zend\Locale $locale (Optional) Locale for territory translation
     * @return array
     * @deprecated
     */
    ...
    public static function getTranslation($value = null, $path = null, $locale = null)
    {
        ...
        $locale = self::findLocale($locale);
        $result = Com\Zend\Locale\Data::getContent($locale, $path, $value);
     ...
     *
     * @param  string|Com\Zend\Locale $locale (Optional) Locale for language translation (defaults to $this locale)
     * @return array
     */
    public static function getQuestion($locale = null)
    {
        ...
        $locale            = self::findLocale($locale);
        $quest             = Com\Zend\Locale\Data::getList($locale, 'question');
     ...
     *
     * @param  string|Com\Zend\Locale $locale     Locale to check for
     ...
     *
     * @param string $locale
     * @throws Com\Zend\Locale\Exception When the given locale is no locale or the autodetection fails
     * @return string
     */
    public static function findLocale($locale = null)
    {
        if ($locale === null) {
            ...
            ###if (Zend_Registry::isRegistered('Com\Zend\Locale')) {
            ###    $locale = Zend_Registry::get('Com\Zend\Locale');
            ###}
        }

        if ($locale === null) {
            $locale = new Com\Zend\Locale();
        }

        if (!Com\Zend\Locale::isLocale($locale, true, false)) {
            if (!Com\Zend\Locale::isLocale($locale, false, false)) {
                $locale = Com\Zend\Locale::getLocaleToTerritory($locale);

                if (empty($locale)) {
                    
                    throw new Com\Zend\Locale\Exception("The locale '$locale' is no known locale");
                }
            } else {
                $locale = new Com\Zend\Locale($locale);
            }
        }
    ...
    public static function getCache()
    {
        ...
        return Com\Zend\Locale\Data::getCache();
    }
    ...
    public static function setCache(Zend_Cache_Core $cache)
    {
        ...
        Com\Zend\Locale\Data::setCache($cache);
    }
    ...
    public static function hasCache()
    {
        ...
        return Com\Zend\Locale\Data::hasCache();
    }
    ...
    public static function removeCache()
    {
        ...
        Com\Zend\Locale\Data::removeCache();
    }
    ...
    public static function clearCache($tag = null)
    {
        ...
        Com\Zend\Locale\Data::clearCache($tag);
    }
    ...
    public static function disableCache($flag)
    {
        ...
        Com\Zend\Locale\Data::disableCache($flag);
    }

    /**
     * Internal function, returns a single locale on detection
     *
     * @param  string|Com\Zend\Locale $locale (Optional) Locale to work on
     * @param  boolean            $strict (Optional) Strict preparation
     * @throws Com\Zend\Locale\Exception When no locale is set which is only possible when the class was wrong extended
     * @return string
     */
    private static function _prepareLocale($locale, $strict = false)
    {
        ...
        // This can only happen when someone extends Com\Zend\Locale and erases the default
        if ($locale === null) {
            
            throw new Com\Zend\Locale\Exception('Autodetection of Locale has been failed!');
        }
     ...
     *
     * @param  string|Com\Zend\Locale $locale Locale to work on
     * @return boolean
     */
