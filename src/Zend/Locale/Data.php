...
class Data
{
    ...
    private static function _findRoute($locale, $path, $attribute, $value, &$temp)
    {
        ...
        if (empty(self::$_ldml[(string) $locale])) {
            ...
            if (!file_exists($filename)) {
                ...
                throw new Com\Zend\Locale\Exception("Missing locale file '$filename' for '$locale' locale.");
            }

            self::$_ldml[(string) $locale] = Com\Zend\Xml\Security::scanFile($filename);
        }
     ...
     *
     * @param  string|Com\Zend\Locale $locale Locale to check
     * @return string
     * @throws Com\Zend\Locale\Exception
     */
    private static function _checkLocale($locale)
    {
        ...
        if (!(Com\Zend\Locale::isLocale((string) $locale, null, false))) {
            
            throw new Com\Zend\Locale\Exception("Locale (" . (string) $locale . ") is a unknown locale");
        }

        if (Com\Zend\Locale::isAlias($locale)) {
            // Return a valid CLDR locale so that the XML file can be loaded.
            return Com\Zend\Locale::getAlias($locale);
        }
    ...
    public static function getList($locale, $path, $value = false)
    {
        ...
        if (!isset(self::$_cache) && !self::$_cacheDisabled) {
            ...
            self::$_cache = Com\Zend\Cache::factory(
                'Core',
                'File',
        ...
        $val = urlencode($val);
        $id  = self::_filterCacheId('Com\Zend\LocaleL_' . $locale . '_' . $path . '_' . $val);
        ...
        switch(strtolower($path)) {
            ...
            default :
                ...
                throw new Com\Zend\Locale\Exception("Unknown list ($path) for parsing locale data.");
                break;
        }

        if (isset(self::$_cache)) {
            if (self::$_cacheTags) {
                self::$_cache->save( serialize($temp), $id, array('Com\Zend\Locale'));
            } else {
    ...
    public static function getContent($locale, $path, $value = false)
    {
        ...
        $val = urlencode($val);
        $id  = self::_filterCacheId('Com\Zend\LocaleC_' . $locale . '_' . $path . '_' . $val);
        if (!self::$_cacheDisabled && ($result = self::$_cache->load($id))) {
            return unserialize($result);
        }
        ...
        switch(strtolower($path)) {
            ...
            case 'defaultcalendar':
                $givenLocale = new Com\Zend\Locale($locale);
            ...
            default :
                ...
                throw new Com\Zend\Locale\Exception("Unknown detail ($path) for parsing locale data.");
                break;
        }
     ...
     *
     * @return Com\Zend\Cache_Core The set cache
     */
    ...
    /**
     * Set a cache for Com\Zend\Locale_Data
     *
     * @param Com\Zend\Cache_Core $cache A cache frontend
     */
    public static function setCache(Com\Zend\Cache_Core $cache)
    {
        self::$_cache = $cache;
    ...
    {
        if (self::$_cacheTags) {
            self::$_cache->clean(Com\Zend\Cache::CLEANING_MODE_MATCHING_TAG, array('Com\Zend\Locale'));
        } else {
            self::$_cache->clean(Com\Zend\Cache::CLEANING_MODE_ALL);
        }
    }
    ...
    private static function _getTagSupportForCache()
    {
        ...
        if ($backend instanceof Com\Zend\Cache\Backend\ExtendedInterface) {
