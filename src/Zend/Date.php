...
class Date extends Com\Zend\Date\DateObject
{
     ...
     *
     * @param  string|integer|Com\Zend\Date|array  $date    OPTIONAL Date value or value of date part to set
     *                                                 ,depending on $part. If null the actual time is set
     * @param  string                          $part    OPTIONAL Defines the input format of $date
     * @param  string|Com\Zend\Locale              $locale  OPTIONAL Locale for parsing input
     * @return Com\Zend\Date
     * @throws Com\Zend\Date\Exception
     */
    public function __construct($date = null, $part = null, $locale = null)
    {
        if (is_object($date) and !($date instanceof Com\Zend\TimeSync\Protocol) and
            !($date instanceof Com\Zend\Date)) {
            if ($locale instanceof Com\Zend\Locale) {
                $locale = $date;
                $date   = null;
                $part   = null;
            } else {
        ...
        if (($date !== null) and !is_array($date) and !($date instanceof Com\Zend\TimeSync\Protocol) and
            !($date instanceof Com\Zend\Date) and !defined($date) and Com\Zend\Locale::isLocale($date, true, false)) {
            $locale = $date;
            $date   = null;
            $part   = null;
        } else if (($part !== null) and !defined($part) and Com\Zend\Locale::isLocale($part, true, false)) {
            $locale = $part;
            $part   = null;
        }
        ...
        if ($date instanceof Com\Zend\TimeSync\Protocol) {
            $date = $date->getInfo();
    ...
    public static function setOptions(array $options = array())
    {
        ...
        foreach ($options as $name => $value) {
            ...
            if (array_key_exists($name, self::$_options)) {
                switch($name) {
                    case 'format_type' :
                        if ((strtolower($value) != 'php') && (strtolower($value) != 'iso')) {
                            ...
                            throw new Com\Zend\Date\Exception("Unknown format type ($value) for dates, only 'iso' and 'php' supported", 0, null, $value);
                        }
                        break;
                    case 'fix_dst' :
                        if (!is_bool($value)) {
                            
                            throw new Com\Zend\Date\Exception("'fix_dst' has to be boolean", 0, null, $value);
                        }
                        break;
                    case 'extend_month' :
                        if (!is_bool($value)) {
                            
                            throw new Com\Zend\Date\Exception("'extend_month' has to be boolean", 0, null, $value);
                        }
                        break;
                    case 'cache' :
                        if ($value === null) {
                            ...
                            if (!$value instanceof Com\Zend\Cache\Core) {
                                
                                throw new Com\Zend\Date\Exception("Instance of Zend_Cache expected");
                            }

                            parent::$_cache = $value;
                            parent::$_cacheTags = Com\Zend\Date\DateObject::_getTagSupportForCache();
                            Com\Zend\Locale\Data::setCache($value);
                        }
                        break;
                    case 'timesync' :
                        if ($value === null) {
                            ...
                            if (!$value instanceof Com\Zend\TimeSync\Protocol) {
                                
                                throw new Com\Zend\Date\Exception("Instance of Zend_TimeSync expected");
                            }
                ...
                throw new Com\Zend\Date\Exception("Unknown option: $name = $value");
            }
        }
    }

    /**
     * Returns this object's internal UNIX timestamp (equivalent to Com\Zend\Date::TIMESTAMP).
     ...
     * @param  string|integer|array|Com\Zend\Date  $stamp   Timestamp to calculate, when null the actual timestamp is calculated
     * @return Com\Zend\Date|integer
     * @throws Com\Zend\Date\Exception
     */
    private function _timestamp($calc, $stamp)
    {
        if ($stamp instanceof Com\Zend\Date) {
        ...
        if (is_array($stamp)) {
            if (isset($stamp['timestamp']) === true) {
                ...
                throw new Com\Zend\Date\Exception('no timestamp given in array');
            }
        }
     ...
     *
     * @param  integer|string|array|Com\Zend\Date  $timestamp  Timestamp to set
     * @return Com\Zend\Date Provides a fluent interface
     * @throws Com\Zend\Date\Exception
     */
     ...
     * Adds a timestamp
     *
     * @param  integer|string|array|Com\Zend\Date  $timestamp  Timestamp to add
     * @return Com\Zend\Date Provides a fluent interface
     * @throws Com\Zend\Date\Exception
     */
     ...
     *
     * @param  integer|string|array|Com\Zend\Date  $timestamp  Timestamp to sub
     * @return Com\Zend\Date Provides a fluent interface
     * @throws Com\Zend\Date\Exception
     */
     ...
     *
     * @param  integer|string|array|Com\Zend\Date  $timestamp  Timestamp to compare
    ...
    {
        if (is_object($format)) {
            if ($format instanceof Com\Zend\Locale) {
                $locale = $format;
                $format = null;
            } else {
        ...
        if (is_object($type)) {
            if ($type instanceof Com\Zend\Locale) {
                $locale = $type;
                $type   = null;
            } else {
                $type = (string) $type;
        ...
        if (($format !== null) && !defined($format)
            ...
            && Com\Zend\Locale::isLocale($format, null, false)) {
            $locale = $format;
            $format = null;
        }

        if (($type !== null) and ($type != 'php') and ($type != 'iso') and
            Com\Zend\Locale::isLocale($type, null, false)) {
            $locale = $type;
            $type = null;
        }
        ...
        if ($format === null) {
            $format = Com\Zend\Locale\Format::getDateFormat($locale) . ' ' . Com\Zend\Locale\Format::getTimeFormat($locale);
        } else if (((self::$_options['format_type'] == 'php') && ($type === null)) or ($type == 'php')) {
            $format = Com\Zend\Locale\Format::convertPhpToIsoFormat($format);
        }
     ...
     *
     * @param  string|integer|Com\Zend\Date  $part  OPTIONAL Defines the date or datepart to return as integer
    ...
    public function get($part = null, $locale = null)
    {
        ...
        if (($part !== null) && !defined($part)
            ...
            && Com\Zend\Locale::isLocale($part, null, false)) {
            $locale = $part;
            $part = null;
        }
        ...
        if ($part === null) {
            ...
            $part = Com\Zend\Locale\Format::convertPhpToIsoFormat($part);
        }
    ...
    private function _parseIsoToDate($token, $locale) {
        switch($token) {
            ...
            case self::WEEKDAY_SHORT :
                ...
                $day     = Com\Zend\Locale\Data::getContent($locale, 'day', array('gregorian', 'format', 'wide', $weekday));
            ...
            case self::WEEKDAY :
                $weekday = strtolower($this->date('D', $this->getUnixTimestamp(), false));
                return $this->_toComment(Com\Zend\Locale\Data::getContent($locale, 'day', array('gregorian', 'format', 'wide', $weekday)));
                break;
            ...
            case 'EEEEE' :
                $weekday = strtolower($this->date('D', $this->getUnixTimestamp(), false));
                $day = Com\Zend\Locale\Data::getContent($locale, 'day', array('gregorian', 'format', 'abbreviated', $weekday));
                return $this->_toComment(iconv_substr($day, 0, 1, 'UTF-8'));
                break;

            case self::WEEKDAY_NAME :
                $weekday = strtolower($this->date('D', $this->getUnixTimestamp(), false));
                return $this->_toComment(Com\Zend\Locale\Data::getContent($locale, 'day', array('gregorian', 'format', 'abbreviated', $weekday)));
                break;
            ...
            case self::MONTH_NAME :
                ...
                return $this->_toComment(Com\Zend\Locale\Data::getContent($locale, 'month', array('gregorian', 'format', 'wide', $month)));
                break;
            ...
            case self::MONTH_NAME_SHORT :
                $month = $this->date('n', $this->getUnixTimestamp(), false);
                return $this->_toComment(Com\Zend\Locale\Data::getContent($locale, 'month', array('gregorian', 'format', 'abbreviated', $month)));
                break;
            ...
            case self::MONTH_NAME_NARROW :
                $month = $this->date('n', $this->getUnixTimestamp(), false);
                $mon = Com\Zend\Locale\Data::getContent($locale, 'month', array('gregorian', 'format', 'abbreviated', $month));
            ...
            case self::MERIDIEM :
                ...
                if ($am == 'am') {
                    return $this->_toComment(Com\Zend\Locale\Data::getContent($locale, 'am'));
                }

                return $this->_toComment(Com\Zend\Locale\Data::getContent($locale, 'pm'));
                break;
            ...
            case 'GGG' :
                ...
                if ($year < 0) {
                    return $this->_toComment(Com\Zend\Locale\Data::getContent($locale, 'era', array('gregorian', 'Abbr', '0')));
                }

                return $this->_toComment(Com\Zend\Locale\Data::getContent($locale, 'era', array('gregorian', 'Abbr', '1')));
                break;

            case self::ERA_NARROW :
                $year = $this->date('Y', $this->getUnixTimestamp(), false);
                if ($year < 0) {
                    return $this->_toComment(iconv_substr(Com\Zend\Locale\Data::getContent($locale, 'era', array('gregorian', 'Abbr', '0')), 0, 1, 'UTF-8')) . '.';
                }

                return $this->_toComment(iconv_substr(Com\Zend\Locale\Data::getContent($locale, 'era', array('gregorian', 'Abbr', '1')), 0, 1, 'UTF-8')) . '.';
                break;

            case self::ERA_NAME :
                $year = $this->date('Y', $this->getUnixTimestamp(), false);
                if ($year < 0) {
                    return $this->_toComment(Com\Zend\Locale\Data::getContent($locale, 'era', array('gregorian', 'Names', '0')));
                }

                return $this->_toComment(Com\Zend\Locale\Data::getContent($locale, 'era', array('gregorian', 'Names', '1')));
                break;

            case self::DATES :
                return $this->_toToken(Com\Zend\Locale\Format::getDateFormat($locale), $locale);
                break;

            case self::DATE_FULL :
                return $this->_toToken(Com\Zend\Locale\Data::getContent($locale, 'date', array('gregorian', 'full')), $locale);
                break;

            case self::DATE_LONG :
                return $this->_toToken(Com\Zend\Locale\Data::getContent($locale, 'date', array('gregorian', 'long')), $locale);
                break;

            case self::DATE_MEDIUM :
                return $this->_toToken(Com\Zend\Locale\Data::getContent($locale, 'date', array('gregorian', 'medium')), $locale);
                break;

            case self::DATE_SHORT :
                return $this->_toToken(Com\Zend\Locale\Data::getContent($locale, 'date', array('gregorian', 'short')), $locale);
                break;

            case self::TIMES :
                return $this->_toToken(Com\Zend\Locale\Format::getTimeFormat($locale), $locale);
                break;

            case self::TIME_FULL :
                return $this->_toToken(Com\Zend\Locale\Data::getContent($locale, 'time', 'full'), $locale);
                break;

            case self::TIME_LONG :
                return $this->_toToken(Com\Zend\Locale\Data::getContent($locale, 'time', 'long'), $locale);
                break;

            case self::TIME_MEDIUM :
                return $this->_toToken(Com\Zend\Locale\Data::getContent($locale, 'time', 'medium'), $locale);
                break;

            case self::TIME_SHORT :
                return $this->_toToken(Com\Zend\Locale\Data::getContent($locale, 'time', 'short'), $locale);
                break;

            case self::DATETIME :
                return $this->_toToken(Com\Zend\Locale\Format::getDateTimeFormat($locale), $locale);
                break;

            case self::DATETIME_FULL :
                return $this->_toToken(Com\Zend\Locale\Data::getContent($locale, 'datetime', array('gregorian', 'full')), $locale);
                break;

            case self::DATETIME_LONG :
                return $this->_toToken(Com\Zend\Locale\Data::getContent($locale, 'datetime', array('gregorian', 'long')), $locale);
                break;

            case self::DATETIME_MEDIUM :
                return $this->_toToken(Com\Zend\Locale\Data::getContent($locale, 'datetime', array('gregorian', 'medium')), $locale);
                break;

            case self::DATETIME_SHORT :
                return $this->_toToken(Com\Zend\Locale\Data::getContent($locale, 'datetime', array('gregorian', 'short')), $locale);
                break;
    ...
    private function _getDigitFromName($name)
    {
        switch($name) {
            ...
            default:
                ...
                throw new Com\Zend\Date\Exception('Month ($name) is not a known month');
        }
    }
     ...
     * For a complete list of supported datepart values look into the docu
     *
     * @param  string|integer|array|Com\Zend\Date  $date    Date or datepart to set
     ...
     * For a complete list of supported datepart values look into the docu.
     *
     * @param  string|integer|array|Com\Zend\Date  $date    Date or datepart to add
     ...
     *
     * @param  string|integer|array|Com\Zend\Date  $date    Date or datepart to subtract
     ...
     *
     * @param  string|integer|array|Com\Zend\Date  $date    Date or datepart to compare with the date object
    ...
    /**
     * Returns a new instance of Com\Zend\Date with the selected part copied.
     ...
     * @param  string|Com\Zend\Locale  $locale  OPTIONAL New object's locale.  No adjustments to timezone are made.
     * @return Com\Zend\Date New clone with requested part
     */
    ...
    public function getTimezoneFromString($zone)
    {
        ...
        if ($zone instanceof Com\Zend\Date) {
     ...
     * @return integer|string|Com\Zend\Date  new timestamp or Com\Zend\Date depending on calculation
     */
    private function _assign($calc, $date, $comp = 0, $dst = false)
    {
        switch ($calc) {
            case 'set' :
                if (!empty($comp)) {
                    $this->setUnixTimestamp(call_user_func(Com\Zend\Locale\Math::$sub, $this->getUnixTimestamp(), $comp));
                }
                $this->setUnixTimestamp(call_user_func(Com\Zend\Locale\Math::$add, $this->getUnixTimestamp(), $date));
            ...
            case 'add' :
                ...
                break;
            case 'sub' :
                $this->setUnixTimestamp(call_user_func(Com\Zend\Locale\Math::$sub, $this->getUnixTimestamp(), $date));
                $value = $this->getUnixTimestamp();
                break;
            default :
                // cmp - compare
                return call_user_func(Com\Zend\Locale\Math::$comp, $comp, $date);
                break;
        }
     ...
     * @param  string|integer|array|Com\Zend\Date  $date    Date or datepart to calculate with
     * @param  string                          $part    Part of the date to calculate, if null the timestamp is used
     * @param  string|Com\Zend\Locale              $locale  Locale for parsing input
     * @return integer|string|Com\Zend\Date        new timestamp
     * @throws Com\Zend\Date\Exception
     */
    private function _calculate($calc, $date, $part, $locale)
    {
        if ($date === null) {
            ...
            throw new Com\Zend\Date\Exception('parameter $date must be set, null is not allowed');
        }

        if (($part !== null) && (strlen($part) !== 2) && (Com\Zend\Locale::isLocale($part, null, false))) {
            $locale = $part;
            $part   = null;
        }
        ...
        if ($date instanceof Com\Zend\Date) {
            ...
            if (empty($part) === false) {
                switch($part) {
                    ...
                    default:
                        ...
                        throw new Com\Zend\Date\Exception("datepart for part ($part) not found in array");
                        break;
                }
            } else {
                $hours = 0;
        ...
        switch($part) {
            ...
            case self::DAY:
                ...
                throw new Com\Zend\Date\Exception("invalid date ($date) operand, day expected", 0, null, $date);
                break;

            case self::WEEKDAY_SHORT:
                $daylist = Com\Zend\Locale\Data::getList($locale, 'day');
                ...
                throw new Com\Zend\Date\Exception("invalid date ($date) operand, weekday expected", 0, null, $date);
                break;
            ...
            case self::DAY_SUFFIX:
                ...
                throw new Com\Zend\Date\Exception('day suffix not supported', 0, null, $date);
                break;
            ...
            case self::WEEKDAY_NARROW:
                $daylist = Com\Zend\Locale\Data::getList($locale, 'day', array('gregorian', 'format', 'abbreviated'));
                $weekday = (int) $this->toString(self::WEEKDAY_DIGIT, 'iso', $locale);
                $cnt = 0;
                foreach ($daylist as $key => $value) {
            ...
            case self::WEEK:
                ...
                throw new Com\Zend\Date\Exception("invalid date ($date) operand, week expected", 0, null, $date);
                break;

            // month formats
            case self::MONTH_NAME:
                $monthlist = Com\Zend\Locale\Data::getList($locale, 'month');
                $cnt = 0;
                ...
                throw new Com\Zend\Date\Exception("invalid date ($date) operand, month expected", 0, null, $date);
                break;
            ...
            case self::MONTH_NAME_SHORT:
                $monthlist = Com\Zend\Locale\Data::getList($locale, 'month', array('gregorian', 'format', 'abbreviated'));
                $cnt = 0;
                foreach ($monthlist as $key => $value) {
                    if (strtoupper($value) == strtoupper($date)) {
                        $found = $key;
            ...
            case self::MONTH_DAYS:
                ...
                throw new Com\Zend\Date\Exception('month days not supported', 0, null, $date);
                break;

            case self::MONTH_NAME_NARROW:
                $monthlist = Com\Zend\Locale\Data::getList($locale, 'month', array('gregorian', 'stand-alone', 'narrow'));
                $cnt       = 0;
                foreach ($monthlist as $key => $value) {
                    if (strtoupper($value) === strtoupper($date)) {
                        $found = $key;
            ...
            case self::LEAPYEAR:
                ...
                throw new Com\Zend\Date\Exception('leap year not supported', 0, null, $date);
                break;
            ...
            case self::YEAR_8601:
                ...
                throw new Com\Zend\Date\Exception("invalid date ($date) operand, year expected", 0, null, $date);
                break;
            ...
            case self::MERIDIEM:
                ...
                throw new Com\Zend\Date\Exception('meridiem not supported', 0, null, $date);
                break;
            ...
            case self::SWATCH:
                ...
                throw new Com\Zend\Date\Exception("invalid date ($date) operand, swatchstamp expected", 0, null, $date);
                break;
            ...
            case self::HOUR_SHORT_AM:
                ...
                throw new Com\Zend\Date\Exception("invalid date ($date) operand, hour expected", 0, null, $date);
                break;
            ...
            case self::MINUTE:
                ...
                throw new Com\Zend\Date\Exception("invalid date ($date) operand, minute expected", 0, null, $date);
                break;
            ...
            case self::SECOND:
                ...
                throw new Com\Zend\Date\Exception("invalid date ($date) operand, second expected", 0, null, $date);
                break;
            ...
            case self::MILLISECOND:
                ...
                throw new Com\Zend\Date\Exception("invalid date ($date) operand, milliseconds expected", 0, null, $date);
                break;
            ...
            case self::TIMEZONE_SECS:
                ...
                throw new Com\Zend\Date\Exception('timezone not supported', 0, null, $date);
                break;

            case self::DAYLIGHT:
                
                throw new Com\Zend\Date\Exception('daylight not supported', 0, null, $date);
                break;

            case self::GMT_DIFF:
            case self::GMT_DIFF_SEP:
                
                throw new Com\Zend\Date\Exception('gmtdiff not supported', 0, null, $date);
                break;
            ...
            case self::ISO_8601:
                ...
                if (empty($datematch) and empty($timematch)) {
                    ...
                    throw new Com\Zend\Date\Exception("unsupported ISO8601 format ($date)", 0, null, $date);
                }
                if (!empty($timematch)) {
            ...
            case self::RFC_2822:
                ...
                if (!$result) {
                    ...
                    throw new Com\Zend\Date\Exception("no RFC 2822 format ($date)", 0, null, $date);
                }
            ...
            case self::TIMESTAMP:
                ...
                throw new Com\Zend\Date\Exception("invalid date ($date) operand, timestamp expected", 0, null, $date);
                break;
            ...
            case self::ERA_NAME:
                ...
                throw new Com\Zend\Date\Exception('era not supported', 0, null, $date);
                break;

            case self::DATES:
                try {
                    $parsed = Com\Zend\Locale\Format::getDate($date, array('locale' => $locale, 'format_type' => 'iso', 'fix_date' => true));
                ...
                } catch (Com\Zend\Locale\Exception $e) {
                    
                    throw new Com\Zend\Date\Exception($e->getMessage(), 0, $e, $date);
                }
                break;

            case self::DATE_FULL:
                try {
                    $format = Com\Zend\Locale\Data::getContent($locale, 'date', array('gregorian', 'full'));
                    $parsed = Com\Zend\Locale\Format::getDate($date, array('date_format' => $format, 'format_type' => 'iso', 'locale' => $locale));
            ...
            case self::DATE_LONG:
                try {
                    $format = Com\Zend\Locale\Data::getContent($locale, 'date', array('gregorian', 'long'));
                    $parsed = Com\Zend\Locale\Format::getDate($date, array('date_format' => $format, 'format_type' => 'iso', 'locale' => $locale));
            ...
            case self::DATE_MEDIUM:
                try {
                    $format = Com\Zend\Locale\Data::getContent($locale, 'date', array('gregorian', 'medium'));
                    $parsed = Com\Zend\Locale\Format::getDate($date, array('date_format' => $format, 'format_type' => 'iso', 'locale' => $locale));
            ...
            case self::DATE_SHORT:
                try {
                    $format = Com\Zend\Locale\Data::getContent($locale, 'date', array('gregorian', 'short'));
                    $parsed = Com\Zend\Locale\Format::getDate($date, array('date_format' => $format, 'format_type' => 'iso', 'locale' => $locale));
            ...
            case self::TIMES:
                try {
                    ...
                    $parsed = Com\Zend\Locale\Format::getTime($date, array('locale' => $locale, 'format_type' => 'iso', 'fix_date' => true));
            ...
            case self::TIME_FULL:
                try {
                    $format = Com\Zend\Locale\Data::getContent($locale, 'time', array('gregorian', 'full'));
                    $parsed = Com\Zend\Locale\Format::getTime($date, array('date_format' => $format, 'format_type' => 'iso', 'locale' => $locale));
                    if ($calc != 'set') {
                        $month = 1;
                        $day   = 1;
                        $year  = 1970;
            ...
            case self::TIME_LONG:
                try {
                    $format = Com\Zend\Locale\Data::getContent($locale, 'time', array('gregorian', 'long'));
                    $parsed = Com\Zend\Locale\Format::getTime($date, array('date_format' => $format, 'format_type' => 'iso', 'locale' => $locale));
                    if ($calc != 'set') {
                        $month = 1;
                        $day   = 1;
            ...
            case self::TIME_MEDIUM:
                try {
                    $format = Com\Zend\Locale\Data::getContent($locale, 'time', array('gregorian', 'medium'));
                    $parsed = Com\Zend\Locale\Format::getTime($date, array('date_format' => $format, 'format_type' => 'iso', 'locale' => $locale));
                    if ($calc != 'set') {
                        $month = 1;
                        $day   = 1;
            ...
            case self::TIME_SHORT:
                try {
                    $format = Com\Zend\Locale\Data::getContent($locale, 'time', array('gregorian', 'short'));
                    $parsed = Com\Zend\Locale\Format::getTime($date, array('date_format' => $format, 'format_type' => 'iso', 'locale' => $locale));
                    if ($calc != 'set') {
                        $month = 1;
                        $day   = 1;
            ...
            case self::DATETIME:
                try {
                    $parsed = Com\Zend\Locale\Format::getDateTime($date, array('locale' => $locale, 'format_type' => 'iso', 'fix_date' => true));
                    if (($calc == 'set') || ($calc == 'cmp')) {
                        --$parsed['month'];
                        --$month;
                        --$parsed['day'];
            ...
            case self::DATETIME_FULL:
                try {
                    $format = Com\Zend\Locale\Data::getContent($locale, 'datetime', array('gregorian', 'full'));
                    $parsed = Com\Zend\Locale\Format::getDateTime($date, array('date_format' => $format, 'format_type' => 'iso', 'locale' => $locale));
            ...
            case self::DATETIME_LONG:
                try {
                    $format = Com\Zend\Locale\Data::getContent($locale, 'datetime', array('gregorian', 'long'));
                    $parsed = Com\Zend\Locale\Format::getDateTime($date, array('date_format' => $format, 'format_type' => 'iso', 'locale' => $locale));
            ...
            case self::DATETIME_MEDIUM:
                try {
                    $format = Com\Zend\Locale\Data::getContent($locale, 'datetime', array('gregorian', 'medium'));
                    $parsed = Com\Zend\Locale\Format::getDateTime($date, array('date_format' => $format, 'format_type' => 'iso', 'locale' => $locale));
                    if (($calc == 'set') || ($calc == 'cmp')) {
                        --$parsed['month'];
                        --$month;
            ...
            case self::DATETIME_SHORT:
                try {
                    $format = Com\Zend\Locale\Data::getContent($locale, 'datetime', array('gregorian', 'short'));
                    $parsed = Com\Zend\Locale\Format::getDateTime($date, array('date_format' => $format, 'format_type' => 'iso', 'locale' => $locale));
            ...
            case self::RFC_3339:
                ...
                if (!$result) {
                    ...
                    throw new Com\Zend\Date\Exception("invalid date ($date) operand, ATOM format expected", 0, null, $date);
                }
            ...
            case self::COOKIE:
                ...
                if (!$result) {
                    ...
                    throw new Com\Zend\Date\Exception("invalid date ($date) operand, COOKIE format expected", 0, null, $date);
                }
            ...
            case self::RFC_1036:
                ...
                if (!$result) {
                    ...
                    throw new Com\Zend\Date\Exception("invalid date ($date) operand, RFC 822 date format expected", 0, null, $date);
                }
            ...
            case self::RFC_850:
                ...
                if (!$result) {
                    ...
                    throw new Com\Zend\Date\Exception("invalid date ($date) operand, RFC 850 date format expected", 0, null, $date);
                }
            ...
            case self::RFC_1123:
                ...
                if (!$result) {
                    ...
                    throw new Com\Zend\Date\Exception("invalid date ($date) operand, RFC 1123 date format expected", 0, null, $date);
                }
            ...
            case self::RSS:
                ...
                if (!$result) {
                    ...
                    throw new Com\Zend\Date\Exception("invalid date ($date) operand, RSS date format expected", 0, null, $date);
                }
            ...
            case self::W3C:
                ...
                if (!$result) {
                    ...
                    throw new Com\Zend\Date\Exception("invalid date ($date) operand, W3C date format expected", 0, null, $date);
                }
            ...
            default:
                if (!is_numeric($date) || !empty($part)) {
                    try {
                        if (empty($part)) {
                            $part  = Com\Zend\Locale\Format::getDateFormat($locale) . " ";
                            $part .= Com\Zend\Locale\Format::getTimeFormat($locale);
                        }

                        $parsed = Com\Zend\Locale\Format::getDate($date, array('date_format' => $part, 'locale' => $locale, 'fix_date' => true, 'format_type' => 'iso'));
     ...
     *
     * @param  string|integer|array|Com\Zend\Date  $date    Date or datepart to equal with
     ...
     *
     * @param  string|integer|array|Com\Zend\Date  $date    Date or datepart to compare with
     * @param  string                          $part    OPTIONAL Part of the date to compare, if null the timestamp is used
     * @param  string|Com\Zend\Locale              $locale  OPTIONAL Locale for parsing input
     * @return boolean
     * @throws Com\Zend\Date\Exception
    ...
    /**
     * Returns only the time of the date as new Com\Zend\Date object
     * For example:
     ...
     *
     * @param  string                    $calc    Calculation to make
     * @param  string|integer|array|Com\Zend\Date  $time    Time to calculate with, if null the actual time is taken
     * @param  string                          $format  Timeformat for parsing input
     * @param  string|Com\Zend\Locale              $locale  Locale for parsing input
     * @return integer|Com\Zend\Date  new time
     * @throws Com\Zend\Date\Exception
     */
    private function _time($calc, $time, $format, $locale)
    {
        if ($time === null) {
            ...
            throw new Com\Zend\Date\Exception('parameter $time must be set, null is not allowed');
        }

        if ($time instanceof Com\Zend\Date) {
            ...
            if (is_array($time)) {
                if ((isset($time['hour']) === true) or (isset($time['minute']) === true) or
                    ...
                    throw new Com\Zend\Date\Exception("no hour, minute or second given in array");
                }
            } else {
                if (self::$_options['format_type'] == 'php') {
                    $format = Com\Zend\Locale\Format::convertPhpToIsoFormat($format);
                ...
                try {
                    ...
                    $parsed = Com\Zend\Locale\Format::getTime($time, array('date_format' => $format, 'locale' => $locale, 'format_type' => 'iso'));
                } catch (Com\Zend\Locale\Exception $e) {
                    
                    throw new Com\Zend\Date\Exception($e->getMessage(), 0, $e);
                }
            }
     ...
     *
     * @param  string|integer|array|Com\Zend\Date  $time    Time to set
     ...
     *
     * @param  string|integer|array|Com\Zend\Date  $time    Time to add
     * @param  string                          $format  OPTIONAL Timeformat for parsing input
     * @param  string|Com\Zend\Locale              $locale  OPTIONAL Locale for parsing input
     * @return Com\Zend\Date Provides a fluent interface
     * @throws Com\Zend\Date\Exception
     ...
     * For example: HH:mm:ss -> 10 -> -10 hours
     *
     * @param  string|integer|array|Com\Zend\Date  $time    Time to sub
     * @param  string                          $format  OPTIONAL Timeformat for parsing input
     * @param  string|Com\Zend\Locale              $locale  OPTIONAL Locale for parsing input
     * @return Com\Zend\Date Provides a fluent inteface
     * @throws Com\Zend\Date\Exception
     */
     ...
     * For example: HH:mm:ss -> 10 -> 10 hours
     *
     * @param  string|integer|array|Com\Zend\Date  $time    Time to compare
     * @param  string                          $format  OPTIONAL Timeformat for parsing input
     * @param  string|Com\Zend\Locale              $locale  OPTIONAL Locale for parsing input
     * @return integer  0 = equal, 1 = later, -1 = earlier
     * @throws Com\Zend\Date\Exception
     ...
     *
     * @param  string                          $calc    Calculation to make
     * @param  string|integer|array|Com\Zend\Date  $date    Date to calculate with, if null the actual date is taken
     * @param  string                          $format  Date format for parsing
     * @param  string|Com\Zend\Locale              $locale  Locale for parsing input
     * @return integer|Com\Zend\Date  new date
     * @throws Com\Zend\Date\Exception
     */
        ...
        if ($date instanceof Com\Zend\Date) {
            ...
            if (is_array($date)) {
                if ((isset($date['year']) === true) or (isset($date['month']) === true) or
                    ...
                    throw new Com\Zend\Date\Exception("no day,month or year given in array");
                }
            } else {
                ...
                try {
                    ...
                    $parsed = Com\Zend\Locale\Format::getDate($date, array('date_format' => $format, 'locale' => $locale, 'format_type' => 'iso'));
     ...
     *
     * @param  string|integer|array|Com\Zend\Date  $date    Date to set
     ...
     *
     * @param  string|integer|array|Com\Zend\Date  $date    Date to add
     ...
     *
     * @param  string|integer|array|Com\Zend\Date  $date    Date to sub
     * @param  string                          $format  OPTIONAL Date format for parsing input
     * @param  string|Com\Zend\Locale              $locale  OPTIONAL Locale for parsing input
     * @return Com\Zend\Date Provides a fluent interface
     * @throws Com\Zend\Date\Exception
     ...
     *
     * @param  string|integer|array|Com\Zend\Date  $date    Date to compare
     * @param  string                          $format  OPTIONAL Date format for parsing input
     * @param  string|Com\Zend\Locale              $locale  OPTIONAL Locale for parsing input
     * @return integer  0 = equal, 1 = later, -1 = earlier
     * @throws Com\Zend\Date\Exception
     ...
     *
     * @param  string|integer|Com\Zend\Date  $date    ISO Date to set
     * @param  string|Com\Zend\Locale        $locale  OPTIONAL Locale for parsing input
     * @return Com\Zend\Date Provides a fluent interface
     * @throws Com\Zend\Date\Exception
     */
     ...
     * Returned is the new date object
     *
     * @param  string|integer|Com\Zend\Date  $date    ISO Date to add
     * @param  string|Com\Zend\Locale        $locale  OPTIONAL Locale for parsing input
     * @return Com\Zend\Date Provides a fluent interface
     * @throws Com\Zend\Date\Exception
     */
     ...
     * Returned is the new date object
     *
     * @param  string|integer|Com\Zend\Date  $date    ISO Date to sub
     * @param  string|Com\Zend\Locale        $locale  OPTIONAL Locale for parsing input
     * @return Com\Zend\Date Provides a fluent interface
     * @throws Com\Zend\Date\Exception
     */
     ...
     * Returned is the new date object
     *
     * @param  string|integer|Com\Zend\Date  $date    RFC 822 to set
     * @param  string|Com\Zend\Locale        $locale  OPTIONAL Locale for parsing input
     * @return Com\Zend\Date Provides a fluent interface
     * @throws Com\Zend\Date\Exception
     */
     ...
     * Returned is the new date object
     *
     * @param  string|integer|Com\Zend\Date  $date    RFC 822 Date to add
     * @param  string|Com\Zend\Locale        $locale  OPTIONAL Locale for parsing input
     * @return Com\Zend\Date Provides a fluent interface
     * @throws Com\Zend\Date\Exception
     */
     ...
     * Returned is the new date object
     *
     * @param  string|integer|Com\Zend\Date  $date    RFC 822 Date to sub
     * @param  string|Com\Zend\Locale        $locale  OPTIONAL Locale for parsing input
     * @return Com\Zend\Date Provides a fluent interface
     * @throws Com\Zend\Date\Exception
     */
    ...
    private function _checkLocation($location)
    {
        if (!isset($location['longitude']) or !isset($location['latitude'])) {
            ...
            throw new Com\Zend\Date\Exception('Location must include \'longitude\' and \'latitude\'', 0, null, $location);
        }
        if (($location['longitude'] > 180) or ($location['longitude'] < -180)) {
            
            throw new Com\Zend\Date\Exception('Longitude must be between -180 and 180', 0, null, $location);
        }
        if (($location['latitude'] > 90) or ($location['latitude'] < -90)) {
            
            throw new Com\Zend\Date\Exception('Latitude must be between -90 and 90', 0, null, $location);
        }
     ...
     * For a list of cities and correct locations use the class Com\Zend\Date_Cities
     *
     ...
     *
     * @param  integer|array|Com\Zend\Date $year Year to check
     * @throws Com\Zend\Date\Exception
     * @return boolean
     */
    public static function checkLeapYear($year)
    {
        if ($year instanceof Com\Zend\Date) {
        ...
        if (is_array($year)) {
            if (isset($year['year']) === true) {
                ...
                throw new Com\Zend\Date\Exception("no year given in array");
            }
        }

        if (!is_numeric($year)) {
            
            throw new Com\Zend\Date\Exception("year ($year) has to be integer for checkLeapYear()", 0, null, $year);
        }
    ...
    {
        return new Com\Zend\Date(time(), self::TIMESTAMP, $locale);
    }
     ...
     *
     * @param  string                          $calc    Calculation to make
     * @param  string|integer|array|Com\Zend\Date  $date    Date or Part to calculate
     ...
     *
     * @param  string                   $calc   Calculation to make
     * @param  string|integer|Com\Zend\Date $value  Datevalue to calculate with, if null the actual value is taken
     * @param  string                   $type
    ...
    private function _calcvalue($calc, $value, $type, $parameter, $locale)
    {
        if ($value === null) {
            ...
            throw new Com\Zend\Date\Exception("parameter $type must be set, null is not allowed");
        }
        ...
        if ($value instanceof Com\Zend\Date) {
            // extract value from object
            $value = $value->toString($parameter, 'iso', $locale);
        } else if (!is_array($value) && !is_numeric($value) && ($type != 'iso') && ($type != 'arpa')) {
            
            throw new Com\Zend\Date\Exception("invalid $type ($value) operand", 0, null, $value);
        }
     ...
     * Returned is the new date object
     *
     * @param  string|integer|array|Com\Zend\Date  $year    Year to set
     * @param  string|Com\Zend\Locale              $locale  OPTIONAL Locale for parsing input
     * @return Com\Zend\Date Provides a fluent interface
     * @throws Com\Zend\Date\Exception
     */
     ...
     * Returned is the new date object
     *
     * @param  string|integer|array|Com\Zend\Date  $year    Year to add
     * @param  string|Com\Zend\Locale              $locale  OPTIONAL Locale for parsing input
     * @return Com\Zend\Date Provides a fluent interface
     * @throws Com\Zend\Date\Exception
     */
     ...
     * Returned is the new date object
     *
     * @param  string|integer|array|Com\Zend\Date  $year    Year to sub
     * @param  string|Com\Zend\Locale              $locale  OPTIONAL Locale for parsing input
     * @return Com\Zend\Date Provides a fluent interface
     * @throws Com\Zend\Date\Exception
     */
     ...
     * Returns if equal, earlier or later
     *
     * @param  string|integer|array|Com\Zend\Date  $year    Year to compare
     * @param  string|Com\Zend\Locale              $locale  OPTIONAL Locale for parsing input
     * @return integer  0 = equal, 1 = later, -1 = earlier
     * @throws Com\Zend\Date\Exception
     */
     ...
     *
     * @param  string                          $calc    Calculation to make
     * @param  string|integer|array|Com\Zend\Date  $month   Month to calculate with, if null the actual month is taken
     * @param  string|Com\Zend\Locale              $locale  Locale for parsing input
     * @return integer|Com\Zend\Date  new time
     * @throws Com\Zend\Date\Exception
     */
    private function _month($calc, $month, $locale)
    {
        if ($month === null) {
            ...
            throw new Com\Zend\Date\Exception('parameter $month must be set, null is not allowed');
        }
        ...
        if ($month instanceof Com\Zend\Date) {
            ...
            if (is_numeric($month)) {
                ...
                if (isset($month['month']) === true) {
                    ...
                    throw new Com\Zend\Date\Exception("no month given in array");
                }
            } else {
                $monthlist  = Com\Zend\Locale\Data::getList($locale, 'month');
                $monthlist2 = Com\Zend\Locale\Data::getList($locale, 'month', array('gregorian', 'format', 'abbreviated'));
                ...
                if ($found == 0) {
                    ...
                    throw new Com\Zend\Date\Exception("unknown month name ($month)", 0, null, $month);
                }
            }
        }
     ...
     * Returned is the new date object
     *
     * @param  string|integer|array|Com\Zend\Date  $month   Month to set
     * @param  string|Com\Zend\Locale              $locale  OPTIONAL Locale for parsing input
     * @return Com\Zend\Date Provides a fluent interface
     * @throws Com\Zend\Date\Exception
     */
     ...
     * Returned is the new date object
     *
     * @param  string|integer|array|Com\Zend\Date  $month   Month to add
     * @param  string|Com\Zend\Locale              $locale  OPTIONAL Locale for parsing input
     * @return Com\Zend\Date Provides a fluent interface
     * @throws Com\Zend\Date\Exception
     */
     ...
     * Returned is the new date object
     *
     * @param  string|integer|array|Com\Zend\Date  $month   Month to sub
     * @param  string|Com\Zend\Locale              $locale  OPTIONAL Locale for parsing input
     * @return Com\Zend\Date Provides a fluent interface
     * @throws Com\Zend\Date\Exception
     */
     ...
     * Returns if equal, earlier or later
     *
     * @param  string|integer|array|Com\Zend\Date  $month   Month to compare
     * @param  string|Com\Zend\Locale              $locale  OPTIONAL Locale for parsing input
     * @return integer  0 = equal, 1 = later, -1 = earlier
     * @throws Com\Zend\Date\Exception
     */
     ...
     *
     * @param Com\Zend\Locale $locale OPTIONAL Locale for parsing input
     * @return Com\Zend\Date
     */
     ...
     *
     * @param string      $calc   Type of calculation to make
     * @param Com\Zend\Date   $day    Day to calculate, when null the actual day is calculated
     * @param Com\Zend\Locale $locale Locale for parsing input
     * @throws Com\Zend\Date\Exception
     * @return Com\Zend\Date|integer
     */
    private function _day($calc, $day, $locale)
    {
        if ($day === null) {
            ...
            throw new Com\Zend\Date\Exception('parameter $day must be set, null is not allowed');
        }
        ...
        if ($day instanceof Com\Zend\Date) {
        ...
        if (is_numeric($day)) {
            ...
            if (isset($day['day']) === true) {
                ...
                throw new Com\Zend\Date\Exception("no day given in array");
            }
        } else {
     ...
     *
     * @param  string|integer|array|Com\Zend\Date  $day     Day to set
     * @param  string|Com\Zend\Locale              $locale  OPTIONAL Locale for parsing input
     * @return Com\Zend\Date Provides a fluent interface
     * @throws Com\Zend\Date\Exception
     */
     ...
     * set locale.
     *
     * @param  string|integer|array|Com\Zend\Date  $day     Day to add
     * @param  string|Com\Zend\Locale              $locale  OPTIONAL Locale for parsing input
     * @return Com\Zend\Date Provides a fluent interface
     * @throws Com\Zend\Date\Exception
     */
     ...
     * set locale.
     *
     * @param  string|integer|array|Com\Zend\Date  $day     Day to sub
     * @param  string|Com\Zend\Locale              $locale  OPTIONAL Locale for parsing input
     * @return Com\Zend\Date Provides a fluent interface
     * @throws Com\Zend\Date\Exception
     */
     ...
     * Returns if equal, earlier or later
     *
     * @param  string|integer|array|Com\Zend\Date  $day     Day to compare
     * @param  string|Com\Zend\Locale              $locale  OPTIONAL Locale for parsing input
     * @return integer  0 = equal, 1 = later, -1 = earlier
     * @throws Com\Zend\Date\Exception
     */
     ...
     *
     * @param  string      $calc     Type of calculation to make
     * @param  Com\Zend\Date   $weekday  Weekday to calculate, when null the actual weekday is calculated
     * @param  Com\Zend\Locale $locale   Locale for parsing input
     * @return Com\Zend\Date|integer
     * @throws Com\Zend\Date\Exception
     */
    private function _weekday($calc, $weekday, $locale)
    {
        if ($weekday === null) {
            ...
            throw new Com\Zend\Date\Exception('parameter $weekday must be set, null is not allowed');
        }
        ...
        if ($weekday instanceof Com\Zend\Date) {
        ...
        if (is_numeric($weekday)) {
            ...
            if (isset($weekday['weekday']) === true) {
                ...
                throw new Com\Zend\Date\Exception("no weekday given in array");
            }
        } else {
     ...
     *
     * @param  string|integer|array|Com\Zend\Date  $weekday Weekday to set
     * @param  string|Com\Zend\Locale              $locale  OPTIONAL Locale for parsing input
     * @return Com\Zend\Date Provides a fluent interface
     * @throws Com\Zend\Date\Exception
     */
     ...
     * wednesday.
     *
     * @param  string|integer|array|Com\Zend\Date  $weekday Weekday to add
     * @param  string|Com\Zend\Locale              $locale  OPTIONAL Locale for parsing input
     * @return Com\Zend\Date Provides a fluent interface
     * @throws Com\Zend\Date\Exception
     */
     ...
     * wednesday.
     *
     * @param  string|integer|array|Com\Zend\Date  $weekday Weekday to sub
     * @param  string|Com\Zend\Locale              $locale  OPTIONAL Locale for parsing input
     * @return Com\Zend\Date Provides a fluent interface
     * @throws Com\Zend\Date\Exception
     */
     ...
     * Returns if equal, earlier or later
     *
     * @param  string|integer|array|Com\Zend\Date  $weekday  Weekday to compare
     * @param  string|Com\Zend\Locale              $locale   OPTIONAL Locale for parsing input
     * @return integer  0 = equal, 1 = later, -1 = earlier
     * @throws Com\Zend\Date\Exception
     */
     ...
     *
     * @param  string|integer|array|Com\Zend\Date  $day     Day of Year to set
     * @param  string|Com\Zend\Locale              $locale  OPTIONAL Locale for parsing input
     * @return Com\Zend\Date Provides a fluent interface
     * @throws Com\Zend\Date\Exception
     */
     ...
     *
     * @param  string|integer|array|Com\Zend\Date  $day     Day of Year to add
     * @param  string|Com\Zend\Locale              $locale  OPTIONAL Locale for parsing input
     * @return Com\Zend\Date Provides a fluent interface
     * @throws Com\Zend\Date\Exception
     */
     ...
     *
     * @param  string|integer|array|Com\Zend\Date  $day     Day of Year to sub
     * @param  string|Com\Zend\Locale              $locale  OPTIONAL Locale for parsing input
     * @return Com\Zend\Date Provides a fluent interface
     * @throws Com\Zend\Date\Exception
     */
     ...
     * Returns if equal, earlier or later
     *
     * @param  string|integer|array|Com\Zend\Date  $day     Day of Year to compare
     * @param  string|Com\Zend\Locale              $locale  OPTIONAL Locale for parsing input
     * @return integer  0 = equal, 1 = later, -1 = earlier
     * @throws Com\Zend\Date\Exception
     */
     ...
     *
     * @param  string|integer|array|Com\Zend\Date  $hour    Hour to set
     * @param  string|Com\Zend\Locale              $locale  OPTIONAL Locale for parsing input
     * @return Com\Zend\Date Provides a fluent interface
     * @throws Com\Zend\Date\Exception
     */
     ...
     *
     * @param  string|integer|array|Com\Zend\Date  $hour    Hour to add
     * @param  string|Com\Zend\Locale              $locale  OPTIONAL Locale for parsing input
     * @return Com\Zend\Date Provides a fluent interface
     * @throws Com\Zend\Date\Exception
     */
     ...
     *
     * @param  string|integer|array|Com\Zend\Date  $hour    Hour to sub
     * @param  string|Com\Zend\Locale              $locale  OPTIONAL Locale for parsing input
     * @return Com\Zend\Date Provides a fluent interface
     * @throws Com\Zend\Date\Exception
     */
     ...
     * Returns if equal, earlier or later
     *
     * @param  string|integer|array|Com\Zend\Date  $hour    Hour to compare
     * @param  string|Com\Zend\Locale              $locale  OPTIONAL Locale for parsing input
     * @return integer  0 = equal, 1 = later, -1 = earlier
     * @throws Com\Zend\Date\Exception
     */
     ...
     *
     * @param  string|integer|array|Com\Zend\Date  $minute  Minute to set
     * @param  string|Com\Zend\Locale              $locale  OPTIONAL Locale for parsing input
     * @return Com\Zend\Date Provides a fluent interface
     * @throws Com\Zend\Date\Exception
     */
     ...
     *
     * @param  string|integer|array|Com\Zend\Date  $minute  Minute to add
     * @param  string|Com\Zend\Locale              $locale  OPTIONAL Locale for parsing input
     * @return Com\Zend\Date Provides a fluent interface
     * @throws Com\Zend\Date\Exception
     */
     ...
     *
     * @param  string|integer|array|Com\Zend\Date  $minute  Minute to sub
     * @param  string|Com\Zend\Locale              $locale  OPTIONAL Locale for parsing input
     * @return Com\Zend\Date Provides a fluent interface
     * @throws Com\Zend\Date\Exception
     */
     ...
     * Returns if equal, earlier or later
     *
     * @param  string|integer|array|Com\Zend\Date  $minute  Hour to compare
     * @param  string|Com\Zend\Locale              $locale  OPTIONAL Locale for parsing input
     * @return integer  0 = equal, 1 = later, -1 = earlier
     * @throws Com\Zend\Date\Exception
     */
     ...
     *
     * @param  string|integer|array|Com\Zend\Date $second Second to set
     * @param  string|Com\Zend\Locale             $locale (Optional) Locale for parsing input
     * @return Com\Zend\Date Provides a fluent interface
     * @throws Com\Zend\Date\Exception
     */
     ...
     *
     * @param  string|integer|array|Com\Zend\Date $second Second to add
     * @param  string|Com\Zend\Locale             $locale (Optional) Locale for parsing input
     * @return Com\Zend\Date Provides a fluent interface
     * @throws Com\Zend\Date\Exception
     */
     ...
     *
     * @param  string|integer|array|Com\Zend\Date $second Second to sub
     * @param  string|Com\Zend\Locale             $locale (Optional) Locale for parsing input
     * @return Com\Zend\Date Provides a fluent interface
     * @throws Com\Zend\Date\Exception
     */
     ...
     * Returns if equal, earlier or later
     *
     * @param  string|integer|array|Com\Zend\Date $second Second to compare
     * @param  string|Com\Zend\Locale             $locale (Optional) Locale for parsing input
     * @return integer  0 = equal, 1 = later, -1 = earlier
     * @throws Com\Zend\Date\Exception
     */
    ...
    public function setFractionalPrecision($precision)
    {
        if (!intval($precision) or ($precision < 0) or ($precision > 9)) {
            ...
            throw new Com\Zend\Date\Exception("precision ($precision) must be a positive integer less than 10", 0, null, $precision);
        }
     ...
     *
     * @param  integer|Com\Zend\Date $milli     (Optional) Millisecond to set, when null the actual millisecond is set
    ...
    public function setMilliSecond($milli = null, $precision = null)
    {
        if ($milli === null) {
            ...
            throw new Com\Zend\Date\Exception("invalid milli second ($milli) operand", 0, null, $milli);
        }
     ...
     *
     * @param  integer|Com\Zend\Date $milli     (Optional) Millisecond to add, when null the actual millisecond is added
    ...
    public function addMilliSecond($milli = null, $precision = null)
    {
        ...
        if (!is_int($precision) || $precision < 1 || $precision > 9) {
            ...
            throw new Com\Zend\Date\Exception(
     ...
     *
     * @param  integer|Com\Zend\Date $milli     (Optional) Millisecond to sub, when null the actual millisecond is subtracted
     * @param  integer           $precision (Optional) Fractional precision for the given milliseconds
     * @return Com\Zend\Date Provides a fluent interface
     */
     ...
     *
     * @param  integer|Com\Zend\Date  $milli  OPTIONAL Millisecond to compare, when null the actual millisecond is compared
     * @param  integer            $precision  OPTIONAL Fractional precision for the given milliseconds
     * @throws Com\Zend\Date\Exception On invalid input
     * @return integer  0 = equal, 1 = later, -1 = earlier
     */
    public function compareMilliSecond($milli = null, $precision = null)
    {
        ...
        if ($precision === 0) {
            ...
            throw new Com\Zend\Date\Exception('precision is 0');
        }
     ...
     *
     * @param  string|integer|array|Com\Zend\Date  $week    Week to set
     * @param  string|Com\Zend\Locale              $locale  OPTIONAL Locale for parsing input
     * @return Com\Zend\Date Provides a fluent interface
     * @throws Com\Zend\Date\Exception
     */
     ...
     *
     * @param  string|integer|array|Com\Zend\Date  $week    Week to add
     * @param  string|Com\Zend\Locale              $locale  OPTIONAL Locale for parsing input
     * @return Com\Zend\Date Provides a fluent interface
     * @throws Com\Zend\Date\Exception
     */
     ...
     *
     * @param  string|integer|array|Com\Zend\Date  $week    Week to sub
     * @param  string|Com\Zend\Locale              $locale  OPTIONAL Locale for parsing input
     * @return Com\Zend\Date Provides a fluent interface
     * @throws Com\Zend\Date\Exception
     */
     ...
     *
     * @param  string|integer|array|Com\Zend\Date  $week    Week to compare
     * @param  string|Com\Zend\Locale              $locale  OPTIONAL Locale for parsing input
     * @return integer 0 = equal, 1 = later, -1 = earlier
     */
     ...
     *
     * @param  string|Com\Zend\Locale $locale (Optional) Locale for parsing input
     * @throws Com\Zend\Date\Exception When the given locale does not exist
     * @return Com\Zend\Date Provides fluent interface
     */
    public function setLocale($locale = null)
    {
        try {
            $this->_locale = Com\Zend\Locale::findLocale($locale);
        } catch (Com\Zend\Locale\Exception $e) {
     ...
     *
     * @param  string|array|Com\Zend\Date $date   Date to parse for correctness
     * @param  string                 $format (Optional) Format for parsing the date string
     * @param  string|Com\Zend\Locale     $locale (Optional) Locale for parsing date parts
     * @return boolean                True when all date parts are correct
     */
    public static function isDate($date, $format = null, $locale = null)
    {
        if (!is_string($date) && !is_numeric($date) && !($date instanceof Com\Zend\Date) &&
            !is_array($date)) {
            return false;
        }
        ...
        $locale = Com\Zend\Locale::findLocale($locale);

        if ($format === null) {
            $format = Com\Zend\Locale\Format::getDateFormat($locale);
        ...
        if (!is_array($date)) {
            try {
                $parsed = Com\Zend\Locale\Format::getDate($date, array('locale' => $locale,
    ...
    protected static function _getLocalizedToken($token, $locale)
    {
        switch($token) {
            ...
            case self::RFC_2822 :
                ...
                break;
            case self::DATES :
                return Com\Zend\Locale\Data::getContent($locale, 'date');
                break;
            case self::DATE_FULL :
                return Com\Zend\Locale\Data::getContent($locale, 'date', array('gregorian', 'full'));
                break;
            case self::DATE_LONG :
                return Com\Zend\Locale\Data::getContent($locale, 'date', array('gregorian', 'long'));
                break;
            case self::DATE_MEDIUM :
                return Com\Zend\Locale\Data::getContent($locale, 'date', array('gregorian', 'medium'));
                break;
            case self::DATE_SHORT :
                return Com\Zend\Locale\Data::getContent($locale, 'date', array('gregorian', 'short'));
                break;
            case self::TIMES :
                return Com\Zend\Locale\Data::getContent($locale, 'time');
                break;
            case self::TIME_FULL :
                return Com\Zend\Locale\Data::getContent($locale, 'time', array('gregorian', 'full'));
                break;
            case self::TIME_LONG :
                return Com\Zend\Locale\Data::getContent($locale, 'time', array('gregorian', 'long'));
                break;
            case self::TIME_MEDIUM :
                return Com\Zend\Locale\Data::getContent($locale, 'time', array('gregorian', 'medium'));
                break;
            case self::TIME_SHORT :
                return Com\Zend\Locale\Data::getContent($locale, 'time', array('gregorian', 'short'));
                break;
            case self::DATETIME :
                return Com\Zend\Locale\Data::getContent($locale, 'datetime');
                break;
            case self::DATETIME_FULL :
                return Com\Zend\Locale\Data::getContent($locale, 'datetime', array('gregorian', 'full'));
                break;
            case self::DATETIME_LONG :
                return Com\Zend\Locale\Data::getContent($locale, 'datetime', array('gregorian', 'long'));
                break;
            case self::DATETIME_MEDIUM :
                return Com\Zend\Locale\Data::getContent($locale, 'datetime', array('gregorian', 'medium'));
                break;
            case self::DATETIME_SHORT :
                return Com\Zend\Locale\Data::getContent($locale, 'datetime', array('gregorian', 'short'));
                break;
            case self::ATOM :
            case self::RFC_3339 :
            case self::W3C :
