...
class TimeSync implements \IteratorAggregate
{
     ...
     * @param  string       $alias  - OPTIONAL an alias for this timeserver
     * @throws Com\Zend\TimeSync\Exception
     */
    ...
    public function setServer($alias)
    {
        if (isset($this->_timeservers[$alias]) === true) {
            ...
            throw new Com\Zend\TimeSync\Exception("'$alias' does not point to valid timeserver");
        }
    }
    ...
    public static function getOptions($key = null)
    {
        ...
        if (isset(TimeSync::$options[$key]) === true) {
            ...
            throw new Com\Zend\TimeSync\Exception("'$key' does not point to valid option");
        }
    }
    ...
    public function getServer($alias = null)
    {
        if ($alias === null) {
            if (isset($this->_current) && $this->_current !== false) {
                ...
                throw new Com\Zend\TimeSync\Exception('there is no timeserver set');
            }
        }
        if (isset($this->_timeservers[$alias]) === true) {
    ...
    public function getDate($locale = null)
    {
        ...
        foreach ($this->_timeservers as $alias => $server) {
            ...
            } catch (Com\Zend\TimeSync\Exception $e) {
                if (!isset($masterException)) {
                    $masterException = new Com\Zend\TimeSync\Exception('all timeservers are bogus');
                }
    ...
    protected function _addServer($target, $alias)
    {
        ...
        if (!in_array($protocol, $this->_allowedSchemes)) {
            ...
            throw new Com\Zend\TimeSync\Exception("'$protocol' is not a supported protocol");
        }
